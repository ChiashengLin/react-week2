...
function App() {
  ...
  const handleLogin = (e) => {
    ...
    .then((res) => {
      ...
      //發送請求之前先帶入token，之後就會自動帶入
      axios.defaults.headers.common['Authorization'] = token;

      axios.get(`${BASE_URL}/v2/api/${API_PATH}/admin/product`)
  ...
  const checkUserLogin = () => {
    ...
    .catch((error) => alert('使用者未登入'));
  }

  //async+await寫法：
  // const checkUserLogin = async () => {
  //   try {
  //     await axios.post(`${BASE_URL}/v2/admin/check`)
  //     alert('使用者已登入')
  //   } catch (error) {
  //     alert('使用者未登入')
  //   }
